...
- reusable trustor registry уже живёт в `/account/trustors` как optional account-scoped convenience module
- registry может использоваться только как convenience prefill, а не как обязательная runtime dependency document flows
- choose-from-registry, если используется, копирует только snapshot-compatible fields в local document state без live link и без `trustorId` dependency в `documents`
...
- OGP `BBCode` generation использует только persisted document payload, persisted author snapshot и persisted trustor snapshot
- OGP `BBCode` generation использует template-driven renderer для веток `ogp_self` и `ogp_representative`
- OGP `BBCode` renderer строится через normalized render context и не должен читать live registry, `/app` shell state или неперсистентные UI defaults
- OGP evidence в итоговом `BBCode` берётся из persisted evidence snapshot: items сортируются по `sortOrder`, используют `labelSnapshot` и `url`, рендерятся как `[URL='...']labelSnapshot[/URL]` и соединяются запятой
- OGP bottom date берётся на момент generation в timezone Moscow и формате `DD.MM.YYYY`
- OGP signature source всегда author/representative profile; representative trustor snapshot не используется как источник подписи
- trustor registry не может быть live source of truth для generation, даже если trustor был выбран из registry
- OGP generation blocking должен показывать точный checklist по `character / trustor / document`, а не общий vague status
- reusable policy для template/PDF/JPG documents = character-scoped image signature + frozen `signatureSnapshot`
- `attorney_request` уже использует эту policy напрямую
- отдельные renderer-specific исключения не должны тихо становиться reusable default для всех future template documents
...
- grounded document AI v2 reuse-ит existing law/precedent retrieval foundation, но остаётся отдельной editor-centric product line
- grounded document AI v2 первого rollout поддерживается только для:
  - `ogp_complaint.violation_summary`
  - `claims.legal_basis_summary`
  - `claims.requested_relief`
- trustor identity fields, evidence rows, publication metadata, short identity/title fields и full-document rewrite не входят в grounded v2 scope
- grounded document AI сохраняет laws-first policy
- если laws не найдены, но есть usable precedents, grounded document AI может перейти в `precedent_grounded`, не выдавая precedent за норму закона
- если retrieval support незащитим, grounded document AI честно возвращает `insufficient_corpus`, а не придумывает ответ вне confirmed corpus
...
- precedents уже учитываются assistant как отдельный secondary corpus layer
- действует laws-first policy:
  - сначала закон
  - потом precedent layer
  - потом интерпретация
- `obsolete` precedents по умолчанию не участвуют в assistant
- precedents не должны выдаваться за нормы закона и не должны подменять law-grounded answer там, где есть подтверждённая норма
...
- уже выполнено:
  ...
  - `/app` root = compatibility surface с явными ссылками в `/account`, `/account/characters`, `/servers` и `/internal` для `super_admin`
  - shared/internal fallback defaults больше не должны использовать `/app/admin-laws` и `/app/admin-security` как canonical targets; default targets = `/internal/laws` и `/internal/security`
