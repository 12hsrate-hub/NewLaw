...
Этот шаг фиксирует post-MVP legal core для базовой юридической AI-выдачи.
...
- source of truth для правового grounding = `server_id + law_version`
...
`actor_context` нужен для framing ответа, допустимых формулировок и правильной интерпретации фактов.
...
Если запрос смешанный, система выбирает primary intent для генерации, а не пытается неявно вести несколько равноправных сценариев сразу.
...
Для `document_text_improvement` главным source of truth по фактам остаётся `fact ledger`, а retrieved нормы работают как legal guardrails и не дают AI выходить за законодательство выбранного сервера.
...
Legal Core должен придерживаться такой user-facing policy:
...
Сам `self-assessment` не должен превращаться в user-facing warning в рамках шага `16`.
...
- risky cases могут быть скрыто помечены для будущего `AI Quality Review`, но сам review-слой и user-facing warnings ещё не входят в этот шаг
