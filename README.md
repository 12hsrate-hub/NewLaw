...
Для internal/admin линии теперь уже появился и отдельный target contour `/internal`: это foundation-level super_admin zone с shared nav и skeleton routes `/internal/laws`, `/internal/precedents`, `/internal/security`, `/internal/health`. Feature content из transitional `/app/admin-*` туда на этом шаге ещё не переносится, но route architecture и access policy уже отделены от account zone и user-facing модулей.
...
- [src/app/internal](./src/app/internal) — foundation-level internal contour для `super_admin` с shared nav и skeleton routes
