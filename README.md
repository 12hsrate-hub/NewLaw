...
- [src/app/internal](./src/app/internal) — internal contour для `super_admin` с shared nav; `/internal/laws`, `/internal/precedents` и `/internal/security` уже выступают target sections, а `/internal/health` пока остаётся foundation-level route
...
- [src/app/(protected-admin)/app/admin-security](./src/app/%28protected-admin%29/app/admin-security) — transitional `super_admin` экран admin account security, пока ещё не удалённый после появления `/internal/security`
