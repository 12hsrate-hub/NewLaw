...
Следующим user-facing слоем над этими модулями теперь уже появился public server directory `/servers`: он не зависит от `/app` shell, доступен гостю и авторизованному пользователю, показывает compact server availability, assistant availability и viewer-aware documents availability без утечки private document data.
